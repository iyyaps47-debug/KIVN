...
from history_page import show_chat_history_page
...
if "show_history_page" not in st.session_state:
...
@media (prefers-color-scheme: dark) {
    .stTextInput > div > div > input:focus {
        background: #1e3a1e !important;
        box-shadow: 0 0 0 4px rgba(102,187,106,0.2) !important;
    }
}
.stSelectbox > div > div {
    border-radius: 10px !important;
...
# 🆕 Route to chat history page
if st.session_state.show_history_page:
...
if st.session_state.submitted:
    ...
    # 🆕 NEW: Chat History Button (Right Side)
    with col_v3:
        if st.button("📜 Chat History", key="chat_history_btn"):
