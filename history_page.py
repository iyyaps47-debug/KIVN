...
Regular users: View only their chat history (NO PDF download)
Admins: View all users' chats + Download PDF + Auto-logout after 10 minutes inactivity
"""
...
def check_admin_session_timeout():
    """Check if admin session has timed out (10 minutes of inactivity)"""
    
    if not st.session_state.get("is_admin"):
        return False  # Not admin, no timeout needed
    
    # Initialize last activity time on first check
    if "admin_last_activity" not in st.session_state:
        st.session_state.admin_last_activity = datetime.now()
        return False
    
    # Get time since last activity
    last_activity = st.session_state.admin_last_activity
    ...
    if time_elapsed > timedelta(minutes=timeout_minutes):
        ...
        st.error("⏱️ **Session Timeout:** Your admin session expired due to 10 minutes of inactivity.")
        st.info("🔐 **Security Notice:** Your session has been automatically logged out for security reasons. Please login again if needed.")
        return True
...
def show_chat_history_page():
    ...
    if st.session_state.is_admin:
        # Check for session timeout (10 minutes inactivity)
        ...
        last_activity = st.session_state.admin_last_activity
        time_elapsed = datetime.now() - last_activity
        minutes_remaining = 10 - int(time_elapsed.total_seconds() / 60)
        seconds_remaining = 60 - (int(time_elapsed.total_seconds()) % 60)
        
        if minutes_remaining <= 2:
            # Warning when less than 2 minutes remaining
            st.markdown(f"""
            <div class="session-timer-warning">
            ⚠️ **URGENT:** Session expires in {minutes_remaining}:{seconds_remaining:02d} minutes | Auto-logout after 10 minutes of inactivity
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            ⏱️ Session expires in {minutes_remaining}:{seconds_remaining:02d} minutes | Auto-logout after 10 minutes of inactivity
            </div>
            """, unsafe_allow_html=True)
                ...
                if st.button("🔓 Login", use_container_width=True):
                    if admin_id and admin_password:
                        if db.verify_admin(admin_id, admin_password):
                            ...
                            st.session_state.admin_last_activity = datetime.now()  # Initialize timer
